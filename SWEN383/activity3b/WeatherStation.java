...
public class WeatherStation extends Observable implements Runnable {
    ...
    private final Barometer pSensor ;	// Pressure Sensor
    
    private final long PERIOD = 1000 ;      // 1 sec = 1000 ms
    private final int KTOC = -27315 ;       // Kelvin to Celsius conversion.
    private final int KTOF = -45967 ;		// Kelvin to Fahrenheit conversion.
    private final double ITOM = 33.8637526;		//inches to millibar
    
    private int currentReading ;
    private double currentPReading ;
    /*
    ...
    public WeatherStation() {
        ...
        pSensor = new Barometer();
        currentReading = sensor.reading() ;
        currentPReading = pSensor.pressure();
    }
    ...
    public synchronized double getFahrenheit() {
        return (currentReading + KTOF) / 100.0 ;
    }
    
    public synchronized double getInchPressure() {
        return (currentPReading) ;
    }
    
    public synchronized double getmBarPressure() {
        return (currentPReading * ITOM) ;
    }
